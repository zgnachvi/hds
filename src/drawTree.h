...
Pnode * build_ptree_rec(Node<int> * t) {   //////////////
	Pnode * pn;
	if (t == NULL) return NULL;
	pn = (Pnode *)my_alloc(sizeof(Pnode));
	pn->left = build_ptree_rec(t->child[0]);
	pn->right = build_ptree_rec(t->child[1]);
	if (pn->left != NULL) pn->left->parent_dir = -1;
	if (pn->right != NULL) pn->right->parent_dir = 1;

	sprintf(pn->label, "%d(%d)", t->key,t->bf);		/////////აქაა მონაცემები. შეგვიძლია დავამატოთ ფერი და .შ.
...
*/
void pretty_print_tree(Node<int> * t) {			/////////////////
	Pnode *proot;
	int xmin, i;
	if (t == NULL) return;
	proot = build_ptree(t);
